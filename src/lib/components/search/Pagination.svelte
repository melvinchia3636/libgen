...
<ul class="list-style-none flex w-4/5 justify-between my-4">
	<li>
		<button
			class="relative flex items-center rounded-md bg-transparent px-4 py-2.5 pl-2 transition-all duration-300 hover:bg-slate-300/50 dark:hover:bg-zinc-700/50"
	...
	<ul class="list-style-none flex gap-4 items-center">
		{#if parseInt(data.resultsCount) > 0 && urlParams?.get('res')}
			{#each Array.from( { length: Math.ceil(parseInt(data.resultsCount) / parseInt(urlParams?.get('res') || '25')) } )
				.map((_, i) => i)
				.slice(Math.max(currentPage - 5, 0), currentPage + (currentPage < 5 ? 4 - currentPage : 0) + 4) as i}
				<li>
					<button
	...
	<li>
		<button
			class="relative flex items-center rounded-md bg-transparent px-4 pr-2.5 py-2 transition-all duration-300 hover:bg-slate-300/50 dark:hover:bg-zinc-700/50"
			on:click={() => goToPage(currentPage + 1)}
		>
			Next
